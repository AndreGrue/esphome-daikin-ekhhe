...
void DaikinEkhheComponent::setup() { start_uart_cycle(); }
...
void DaikinEkhheComponent::loop() {
  ...
  if (!uart_active_) {
    if (now - last_process_time_ >= update_interval_
  ...
  if (uart_active_ && !packet_set_complete()) {
    if (last_rx_time_ > 0 && (now - last_rx_time_) > kCycleTimeoutMs
        && !cycle_timeout_logged_) {
      cycle_timeouts_++;
      ...
      DAIKIN_WARN(
          TAG,
          "Cycle partial: missing=%s bytes=%u packets=%u crc=%u frame=%u",
          missing.c_str(),
          cycle_bytes_read_,
          cycle_packets_seen_,
...
void DaikinEkhheComponent::store_latest_packet(uint8_t byte) {
  ...
  if (!read_packet_bytes_(
          packet.data() + 1, expected_length - 1, kFrameReadTimeoutMs)) {
    raw_frames_error_++;
...
bool DaikinEkhheComponent::read_packet_bytes_(uint8_t* dest,
                                              size_t   length,
                                              uint32_t timeout_ms) {
...
void DaikinEkhheComponent::store_raw_frame_(uint8_t        packet_type,
                                            const uint8_t* data,
                                            size_t         length,
                                            uint8_t        flags) {
  ...
  if ((flags
       & (RAW_FRAME_CRC_ERROR | RAW_FRAME_TIMEOUT | RAW_FRAME_TRUNCATED
          | RAW_FRAME_UNKNOWN_TYPE))
      != 0) {
    raw_frames_error_++;
  }
}
...
std::string DaikinEkhheComponent::packet_mask_to_string_(uint8_t mask) const {
  std::string out;
  if (mask & kPacketMaskDD)
    out += "DD,";
  if (mask & kPacketMaskD2)
    out += "D2,";
  if (mask & kPacketMaskD4)
    out += "D4,";
  if (mask & kPacketMaskC1)
    out += "C1,";
  if (mask & kPacketMaskCC)
    out += "CC,";
  if (out.empty()) {
    return "none";
  }
...
bool DaikinEkhheComponent::should_publish_float_(
...
bool DaikinEkhheComponent::should_publish_bool_(
    const std::string&               key,
    bool                             value,
...
bool DaikinEkhheComponent::should_publish_text_(
    const std::string&                  key,
...
bool DaikinEkhheComponent::should_publish_debug_text_(
    const std::string& key,
    const std::string& value,
    uint32_t           min_interval_ms) {
  uint32_t now = millis();
...
uint8_t DaikinEkhheComponent::packet_type_from_string_(
    const std::string& value) const {
  if (value == "DD")
    return DD_PACKET_START_BYTE;
  if (value == "D2")
    return D2_PACKET_START_BYTE;
  if (value == "D4")
    return D4_PACKET_START_BYTE;
  if (value == "C1")
    return C1_PACKET_START_BYTE;
  if (value == "CC")
    return CC_PACKET_START_BYTE;
  return 0;
}

std::string
    DaikinEkhheComponent::packet_type_to_string_(uint8_t packet_type) const {
  switch (packet_type) {
  case DD_PACKET_START_BYTE:
    return "DD";
  case D2_PACKET_START_BYTE:
...
const DaikinEkhheComponent::RawFrameEntry*
    DaikinEkhheComponent::find_raw_frame_by_seq_(uint32_t seq,
                                                 size_t&  index) const {
...
const DaikinEkhheComponent::RawFrameEntry*
    DaikinEkhheComponent::find_latest_frame_by_type_(uint8_t packet_type,
                                                     size_t& index,
  ...
  if (packet_type == 0) {
    ...
    for (size_t i = 0; i < raw_frame_count_; ++i) {
      size_t idx =
          (raw_frame_head_ + raw_frame_count_ - 1 - i) % kRawFrameBufferSize;
      const RawFrameEntry& entry = raw_frames_[idx];
...
const DaikinEkhheComponent::RawFrameEntry*
    DaikinEkhheComponent::find_previous_frame_by_type_(uint8_t  packet_type,
                                                       uint32_t seq,
                                                       size_t&  index,
                                                       bool require_ok) const {
...
const DaikinEkhheComponent::RawFrameEntry*
    DaikinEkhheComponent::select_raw_frame_(size_t& index, size_t& back) {
  if (raw_frame_count_ == 0) {
    return nullptr;
  }
  ...
  if (debug_freeze_ && debug_frozen_seq_ != 0) {
    const RawFrameEntry* entry =
        find_raw_frame_by_seq_(debug_frozen_seq_, index);
    if (entry != nullptr) {
      size_t newest_index =
          (raw_frame_head_ + raw_frame_count_ - 1) % kRawFrameBufferSize;
  ...
  const RawFrameEntry* entry =
      find_latest_frame_by_type_(debug_packet_type_, index, true);
  if (entry == nullptr) {
...
std::string
    DaikinEkhheComponent::raw_frame_flags_to_string_(uint8_t flags) const {
  if (flags == 0) {
    return "none";
  }
  std::string out;
  if (flags & RAW_FRAME_CRC_ERROR)
    out += "crc,";
  if (flags & RAW_FRAME_TIMEOUT)
    out += "timeout,";
  if (flags & RAW_FRAME_TRUNCATED)
    out += "truncated,";
  if (flags & RAW_FRAME_UNKNOWN_TYPE)
    out += "unknown,";
  if (!out.empty()) {
    out.pop_back();
  }
...
std::string DaikinEkhheComponent::format_raw_frame_hex_(
    const RawFrameEntry& entry) const {
...
std::string
    DaikinEkhheComponent::format_raw_frame_hex_data_(const uint8_t* data,
                                                     size_t length) const {
  static const char hex[] = "0123456789ABCDEF";
  std::string       out;
...
std::string
    DaikinEkhheComponent::format_raw_frame_meta_(const RawFrameEntry& entry,
                                                 size_t               index,
                                                 size_t               back,
  ...
  snprintf(
      buffer,
      sizeof(buffer),
      "type=0x%02X len=%u flags=%s ts_ms=%u pos=%u back=%u age_ms=%u seq=%u",
      entry.packet_type,
      entry.length,
      flags.c_str(),
      entry.timestamp_ms,
...
bool DaikinEkhheComponent::is_known_offset_(uint8_t packet_type,
                                            size_t  offset,
                                            size_t  length) const {
...
std::string DaikinEkhheComponent::format_unknown_fields_(
    const RawFrameEntry& entry) const {
  std::string out;
  char        header[64];
  snprintf(header,
           sizeof(header),
           "type=0x%02X len=%u unknown:",
           entry.packet_type,
           entry.length);
  out.append(header);
  bool   any  = false;
...
std::string
    DaikinEkhheComponent::format_frame_diff_(const RawFrameEntry& entry,
  ...
  if (prev == nullptr) {
    snprintf(header,
             sizeof(header),
             "type=0x%02X len=%u diff=none (no previous)",
             entry.packet_type,
             entry.length);
  ...
  snprintf(header,
           sizeof(header),
           "type=0x%02X len=%u prev_len=%u diff:",
           entry.packet_type,
           entry.length,
           prev->length);
  out.append(header);
...
void DaikinEkhheComponent::publish_debug_outputs_() {
  ...
  auto     publish_debug_sensor = [this](const std::string& key,
                                     float              value,
                                     uint32_t           min_interval_ms,
                                     uint32_t           refresh_ms) {
    ...
    if (should_publish_float_(key,
                              value,
                              debug_last_published_values_,
                              debug_last_published_values_ms_,
                              min_interval_ms,
                              0.0f,
                              refresh_ms)) {
      defer(
          [this, key, value]() { debug_sensors_[key]->publish_state(value); });
    }
  };

  publish_debug_sensor("frames_captured_total",
                       raw_frames_captured_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("frames_dropped_total",
                       raw_frames_dropped_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("frames_truncated_total",
                       raw_frames_truncated_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("crc_errors_total",
                       raw_crc_errors_total_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("framing_errors_total",
                       raw_framing_errors_total_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("bytes_captured_total",
                       raw_bytes_captured_,
                       kDebugCounterPublishIntervalMs,
                       kDebugCounterPublishIntervalMs);
  publish_debug_sensor("cycle_parse_time_ms",
                       cycle_parse_ms_,
                       kDebugTimingPublishMinIntervalMs,
                       0);
  publish_debug_sensor("cycle_total_time_ms",
                       cycle_total_ms_,
                       kDebugTimingPublishMinIntervalMs,
                       0);
  publish_debug_sensor("cycle_over_budget_total",
  ...
  auto publish_debug_text = [this](const std::string& key,
                                   const std::string& value) {
    if (debug_text_sensors_.find(key) == debug_text_sensors_.end()) {
      return;
    }
    if (should_publish_debug_text_(
            key, value, kDebugTextPublishMinIntervalMs)) {
      defer([this, key, value]() {
        debug_text_sensors_[key]->publish_state(value);
      });
    }
  };

  publish_debug_text("daikin_raw_frame_hex", format_raw_frame_hex_(*entry));
  publish_debug_text("daikin_raw_frame_meta",
                     format_raw_frame_meta_(*entry, index, back, now_ms));
  publish_debug_text("daikin_unknown_fields", format_unknown_fields_(*entry));

  size_t               prev_index = 0;
  const RawFrameEntry* prev       = find_previous_frame_by_type_(
      entry->packet_type, entry->seq, prev_index, true);
  if (prev == nullptr) {
    prev = find_previous_frame_by_type_(
        entry->packet_type, entry->seq, prev_index, false);
  }
...
void DaikinEkhheComponent::publish_cc_snapshot_(const char* override_text) {
  ...
  std::string formatted =
      format_raw_frame_hex_data_(cc_snapshot_data_, cc_snapshot_len_);
...
void DaikinEkhheComponent::process_packet_set() {
  ...
  bool has_required =
      (cycle_packet_types_seen_ & kRequiredPacketMask) == kRequiredPacketMask;
  ...
  // parse_d2_packet(last_d2_packet);  // Don't process D2 since all info is in
  // CC as well
  ...
  if (success) {
    ...
    DAIKIN_HEALTH(TAG,
                  "Cycle ok: bytes=%u packets=%u parsed=%u types=%s "
                  "parse_ms=%u chk_err=%u frm_err=%u",
                  cycle_bytes_read_,
                  cycle_packets_seen_,
                  cycle_packets_parsed_,
                  types.c_str(),
    ...
    if (cycle_framing_errors_ > 0 && !cycle_timeout_logged_) {
      DAIKIN_WARN(
          TAG,
          "Cycle warn: framing=%u last_start=0x%02X bytes=%u packets=%u",
          cycle_framing_errors_,
          cycle_framing_error_start_,
          cycle_bytes_read_,
          cycle_packets_seen_);
    }
    if (!has_required && cycle_checksum_errors_ > 0) {
      std::string types = packet_mask_to_string_(cycle_packet_types_seen_);
      std::string checksum_types =
          packet_mask_to_string_(cycle_checksum_error_mask_);
      DAIKIN_ERROR(TAG,
                   "Cycle error: checksum=%u checksum_types=%s types=%s "
                   "bytes=%u packets=%u",
                   cycle_checksum_errors_,
                   checksum_types.c_str(),
                   types.c_str(),
                   cycle_bytes_read_,
                   cycle_packets_seen_);
    }
...
void DaikinEkhheComponent::register_binary_sensor(
...
void DaikinEkhheComponent::register_number(const std::string&       number_name,
...
void DaikinEkhheComponent::register_select(const std::string& select_name,
...
void DaikinEkhheComponent::register_timestamp_sensor(
    text_sensor::TextSensor* sensor) {
  this->timestamp_sensor_ = sensor;
}

void DaikinEkhheComponent::register_debug_text_sensor(
    const std::string& sensor_name, esphome::text_sensor::TextSensor* sensor) {
  if (sensor != nullptr) {
    debug_text_sensors_[sensor_name] = sensor;
  }
}

void DaikinEkhheComponent::register_debug_sensor(
    const std::string& sensor_name, esphome::sensor::Sensor* sensor) {
  if (sensor != nullptr) {
    debug_sensors_[sensor_name] = sensor;
  }
}

void DaikinEkhheComponent::register_debug_select(
    DaikinEkhheDebugSelect* select) {
  this->debug_packet_select_ = select;
  if (this->debug_packet_select_ != nullptr) {
    this->debug_packet_select_->publish_state(
        packet_type_to_string_(debug_packet_type_));
  }
}
...
void DaikinEkhheComponent::set_sensor_value(const std::string& sensor_name,
                                            float              value) {
  if (sensors_.find(sensor_name) != sensors_.end()) {
    if (!cycle_publish_allowed_) {
      return;
    }
    if (should_publish_float_(sensor_name,
                              value,
                              last_published_sensor_values_,
                              last_published_sensor_ms_,
                              kFastPublishMinIntervalMs,
                              kFloatPublishEpsilon,
                              0)) {
...
void DaikinEkhheComponent::set_binary_sensor_value(
    const std::string& sensor_name, bool value) {
  if (binary_sensors_.find(sensor_name) != binary_sensors_.end()) {
    if (!cycle_publish_allowed_) {
      return;
    }
    if (should_publish_bool_(sensor_name,
                             value,
                             last_published_binary_values_,
                             last_published_binary_ms_,
...
void DaikinEkhheComponent::set_number_value(const std::string& number_name,
                                            float              value) {
  // This sets a number value that's been gotten from the UART stream, not
  // something that's set through the API or UI
  if (numbers_.find(number_name) != numbers_.end()) {
    if (!cycle_publish_allowed_) {
      return;
    }
    if (should_publish_float_(number_name,
                              value,
                              last_published_number_values_,
                              last_published_number_ms_,
                              0,
                              kFloatPublishEpsilon,
                              kSlowPublishRefreshMs)) {
...
void DaikinEkhheComponent::set_select_value(const std::string& select_name,
                                            int                value) {
  // This sets a select value that's been gotten from the UART stream, not
  // something that's set through the API or UI
  if (selects_.count(select_name)) {
    ...
    for (const auto& entry : select->get_select_mappings()) {
      if (entry.second == value) {
        if (should_publish_text_(select_name,
                                 entry.first,
                                 last_published_select_values_,
                                 last_published_select_ms_,
                                 kSlowPublishRefreshMs)) {
          // Update ESPHome with the new selected value
          defer(
              [this, select, entry]() { select->publish_state(entry.first); });
        }
        return;
      }
    }
...
void DaikinEkhheComponent::set_debug_packet(const std::string& value) {
  ...
  if (debug_packet_select_ != nullptr) {
    debug_packet_select_->publish_state(
        packet_type_to_string_(debug_packet_type_));
  }
}
...
void DaikinEkhheComponent::save_cc_snapshot() {
  ...
  size_t               index = 0;
  const RawFrameEntry* entry =
      find_latest_frame_by_type_(CC_PACKET_START_BYTE, index, true);
  if (entry == nullptr) {
...
void DaikinEkhheComponent::restore_cc_snapshot() {
  ...
  std::vector<uint8_t> packet(cc_snapshot_data_,
                              cc_snapshot_data_ + cc_snapshot_len_);
