...
public:
  ...
  // Needed so we can set this from python and then reference it in the control
  // function
...
private:
  ...
  void    store_raw_frame_(uint8_t        packet_type,
                           const uint8_t* data,
                           size_t         length,
                           uint8_t        flags);
  ...
  bool        should_publish_float_(const std::string&               key,
                                    float                            value,
                                    ...
                                    float                            epsilon,
                                    uint32_t                         refresh_ms);
  bool        should_publish_bool_(const std::string&               key,
                                   bool                             value,
                                   std::map<std::string, bool>&     last_values,
                                   std::map<std::string, uint32_t>& last_publish_ms,
                                   uint32_t                         refresh_ms);
  bool        should_publish_text_(const std::string&                  key,
                                   const std::string&                  value,
  ...
  bool                 should_publish_debug_text_(const std::string& key,
                                                  const std::string& value,
                                                  uint32_t           min_interval_ms);
  ...
  const RawFrameEntry* find_raw_frame_by_seq_(uint32_t seq,
                                              size_t&  index) const;
  const RawFrameEntry* find_latest_frame_by_type_(uint8_t packet_type,
                                                  size_t& index,
                                                  bool    require_ok) const;
  const RawFrameEntry* find_previous_frame_by_type_(uint8_t  packet_type,
                                                    uint32_t seq,
                                                    size_t&  index,
                                                    bool     require_ok) const;
  ...
  std::string          format_raw_frame_hex_data_(const uint8_t* data,
                                                  size_t         length) const;
  std::string          format_raw_frame_meta_(const RawFrameEntry& entry,
                                              size_t               index,
                                              size_t               back,
                                              uint32_t             now_ms) const;

  std::string format_unknown_fields_(const RawFrameEntry& entry) const;
  std::string format_frame_diff_(const RawFrameEntry& entry,
  ...
  bool uart_tx_active_ =
      false;  // used for SW "flow control" to avoid RS485 bus contention
...
static const std::map<std::string, uint8_t> U_NUMBER_PARAM_INDEX = {
    ...
    {ELECTRIC_T_TEMPERATURE,
     DaikinEkhheComponent::CC_PACKET_ELECTRIC_TTARGET_IDX},
...
static const std::map<std::string, std::pair<uint8_t, uint8_t>>
    SELECT_BITMASKS = {
